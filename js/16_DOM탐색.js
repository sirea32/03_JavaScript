...
checkBtn1.addEventListener("click", ()=>{
  ...
  const test1 = document.querySelector("#test1"); // ul 유도중(html에 있는 ul태그)

  // 요소.childNodes -> 배열 형태(NodeList)로 반환(한번에 가져오려면 배열형태가 되야함)
  ...
  // list[5]의 이전, 이전 형제 노드 선택 (밑에는 이전의 이전을 보여준 예시)

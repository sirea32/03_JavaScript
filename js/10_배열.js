...
function check1(){
  ...
  // 콘솔 로그만 위에처럼 , 써서 한번에 여러개를 확인가능
  
  /* 배열명.length : 배열의 길이(길이 체크? 0 3 0 3 위기준으로) */
...
*/
function check4(){
  const arr = new Array(5);  // = new Array; 가능 = new Array(); 가능 = []; 가능
          // 5칸짜리 배열 생성
...
function selectMenu(){
  ...
  const randomSelect = Math.floor(Math.random() * menus.length);
  
  // 화면에 #menuResult에 출력하기
  document.getElementById('menuResult').innerText = menus[randomSelect];
}
...
function check7(){
  ...
  // 2차원 배열 선언 및 초기화
  const arr = [
    /* 0행-> arr[0] */ [10, 20, 30, 40],
    /* 1행-> arr[1] */ [50, 60, 70, 80],
    /* 2행-> arr[2] */ [90, 100]
  ];
...
function check8(){
  ...
  // 배열 생성
  const arr = []; // [[],[],[],[]]; 이렇게해도 똑같다
                  // []; 하려면 바로 밑에 행 생성이라는 명령어를 넣어야한다

  // 난수를 2차원 배열에 대입    
  for(let row=0 ; row<4 ; row++){ // 행 반복

    arr[row] = []; // 행 생성 -> 위에 []; 사용하려면 행 생성을 해야한다 

    for(let col=0 ; col<4 ; col++){ // 열 반복

      // 난수 생성
      const randomNumber = Math.floor(Math.random() * 100); 
                       // 1부터 시작이었으면 맨뒤에 * 99)+1 했으면 되었다

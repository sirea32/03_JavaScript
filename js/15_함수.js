...
btn6.addEventListener("click", function(){ // 익명 함수
  ...
  // (요즘 함수 정의 방법 
  // (function 단어 안쓰기가 트렌드다!! 그래서 익명함수를 이용하고있다)  )

  // 해당 함수는 지역 변수에 저장됨 -> 이 지역에서만 사용 가능한 함수가 된다
  //                                -> 해당 지역에서만 사용 가능한 함수
...
// 화살표 함수 기본 형태              크/작거나 같다는 >= <= 왼쪽에있다
arrows[0].addEventListener("click", () => {

  // 익명(화살표) 함수를 변수에 대입            화살표는 익명 함수다
  const sumFn = (a, b, c) => {
    return a + b + c;
  }
...
arrows[2].addEventListener("click", () => {
  ...
  // 하나일때는 생략 되어서 () {} return이 안보이지만 존재함
...
arrows[3].addEventListener("click", ()=>{
  ...
  // 정상 작정 (밑에있는 잘못 작성 예시는 하나라고 생략했다가 저리된거)

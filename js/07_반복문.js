...
function check6(){
  ...
  for(let num = 1 ; num <= 10 ; num+=2){ // num++ -> num = num + 1 (2증가로 바꾸려면?) num = num + 2 -> num+=2
    console.log(num);
  }
...
function check13(){
  ...
  for(let num = 1 ; num <= 20 ; num++){
    ...
    if(num % multiple === 0){
      // console.log(shape);
      str += `${shape} `; // `(백틱)하고 마지막에 무조건 한칸 띄어쓰기(스페이스바 한번 눌러!)
    ...
    if(num === 10) str += "<br>"; // 한 줄 인 경우 { } 생략 가능
...
function check22(){
  ...
  for(let row = 1 ; row <= 3 ; row++){
    ...
    for(let col = 1 ; col <= 4 ; col++){
      ...
      str += count++; // count 값 대입 후 1증가 바로위의 단축버전
    }
    console.log(str);
  }
...
function check23(){
  ...
  let value;
  // 입력된 숫자가 0이 아니면 반복
  while( (value = Number(prompt("숫자 입력"))) != 0 ){ // 우선순위 1. 비교연산자 2. 대입연산자
    sum += value; // 누적
  }

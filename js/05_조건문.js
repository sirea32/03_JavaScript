...
function check2(){
  ...
  // 난수 발생 : Math.random()
  //     ->      0.0 <= 난수 < 1.0 (아무리 나와봤자 0.9999....가 최대이고 1 밑으로만 나온다)
...
// 내방식 보니까 너무 길게함 두번째 방식처럼 줄이면 괜춘할듯
/* function ageCheck(){
...
// 두번째 방법 if 뒤에 조건이 줄어든거 보니 괜춘한듯 그런데 세번째 방법이 최고
function ageCheck(){
...
/* 어린이, 청소년, 성인 판별 ver2 */ /* 첫번째 방법에 단축한것 이게 베스트인가?!! */
function ageCheck2(){   
  const inputAge = document.getElementById("inputAge");
  const value = Number(inputAge.value);
...
/* { } 생략 버전 그런데 사람 스타일이라 해도되고 안해도되고임 회사 바이 회사 가독성을 위해 사람을 위해 사용해도 좋음 */
function ageCheck2(){   
  const inputAge = document.getElementById("inputAge");
  const value = Number(inputAge.value);
...
/* 입력 받은 수가 3의 배수가 맞는지 검사 */ /* 중첩 if문은 많이 지저분해질수있어 요새는 지양하는 추세이다 */
function check4() {
